...
def load(jerk):
    return pd.read_csv(f"results/trajectory/offline_omega_j{jerk}.csv")


def plot_full():
    fig, axes = plt.subplots(4, 1, figsize=(12, 14), sharex=True)
    fig.suptitle("Omega-Max Offline Trajectory Comparison\n(t0 = omega_max, jerk = 0.4, 0.6, 0.8, 1.0)",
    ...
    out = "results/trajectory/omega_comparison_full.png"
...
def plot_brake():
    ...
    fig.suptitle("Brake Phase Detail — Omega-Max Mode Comparison\n(jerk = 0.4, 0.6, 0.8, 1.0)",
                 fontsize=14, fontweight="bold")
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.97])
    out = "results/trajectory/omega_comparison_brake.png"
    plt.savefig(out, dpi=150)
    print(f"Brake comparison saved to: {out}")
    plt.close()

    print("\n" + "=" * 60)
    print("Omega-Max Comparison Summary")
    print("=" * 60)
    for s in stats:
